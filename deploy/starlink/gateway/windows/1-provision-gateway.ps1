...
$tunnelName = [System.IO.Path]::GetFileNameWithoutExtension($TunnelConfigPath)
$serviceName = "WireGuardTunnel`$$tunnelName"
$existingSvc = Get-Service -Name $serviceName -ErrorAction SilentlyContinue
if ($existingSvc) {
    Write-Ok "Service $serviceName already installed (status: $($existingSvc.Status))."
} else {
    & $wgExe /installtunnelservice $TunnelConfigPath
    Start-Sleep -Seconds 2
    $existingSvc = Get-Service -Name $serviceName -ErrorAction SilentlyContinue
    if (-not $existingSvc) { throw "Tunnel service $serviceName did not appear after /installtunnelservice — check $TunnelConfigPath for syntax errors." }
    Write-Ok "Installed and started $serviceName (startup type Automatic by default)."
}
