...
const EN = {
  ...
  'st.vpsHelper':       'VPS Helper (advanced)',
  'st.vpsHelperD':      'For SSH-into-your-own-VPS users: let selected VPS tools (e.g. Claude Code) exit via ReaLink',
  // VPS Helper screen (Case 2 — server-side exit)
  'vps.title':          'VPS Helper',
  'vps.lead':           'For users who SSH into their own VPS (e.g. with Termius) and run tools like Claude Code there.',
  'vps.note':           'That traffic runs on the VPS, not your phone — so app bypass on the phone can’t help, and the VPS’s own IP is often blocked. This gives your VPS a dedicated ReaLink exit. Your SSH connection stays direct; only the tools you point at it use ReaLink.',
  'vps.status':         'Status',
  'vps.state.none':     'Not set up',
  'vps.state.pending':  'Provisioning…',
  'vps.state.active':   'Active',
  'vps.state.revoking': 'Disabling…',
  'vps.state.revoked':  'Disabled',
  'vps.state.error':    'Error',
  'vps.provisioning':   'Provisioning your VPS Helper…',
  'vps.request':        'Request VPS Helper',
  'vps.cmdLabel':       'Run this ONE command on your VPS (paste into your SSH session):',
  'vps.copy':           'Copy install command',
  'vps.after':          'After it finishes, run:  source /usr/local/realink-vps-helper/proxy.env  then start your tool (e.g. claude).',
  'vps.revoke':         'Disable / Revoke',
  'vps.revokeTitle':    'Disable VPS Helper?',
  'vps.revokeConfirm':  'This revokes the VPS’s ReaLink identity. Also run the uninstall command on the VPS to remove it fully.',
  'vps.copiedTitle':    'Copied',
  'vps.copiedBody':     'Install command copied. Paste it into your VPS SSH session.',
  'vps.errorTitle':     'VPS Helper',
  'vps.disclaimer':     'This does not change your VPN or Smart Mode, and does not route Termius. It only sets up a local proxy on your VPS for tools that explicitly use it.',
...
const FA: typeof EN = {
  ...
  'st.vpsHelper':       'دستیار VPS (پیشرفته)',
  'st.vpsHelperD':      'برای کاربرانی که با SSH به VPS خودشان وصل می‌شوند: اجازه بده ابزارهای انتخابی روی VPS (مثل Claude Code) از طریق ReaLink خارج شوند',
  // VPS Helper screen (Case 2 — server-side exit)
  'vps.title':          'دستیار VPS',
  'vps.lead':           'برای کاربرانی که با SSH (مثلاً با Termius) به VPS خودشان وصل می‌شوند و ابزارهایی مثل Claude Code را همان‌جا اجرا می‌کنند.',
  'vps.note':           'آن ترافیک روی VPS اجرا می‌شود، نه روی گوشی شما — پس bypass اپ روی گوشی کمکی نمی‌کند و IP خود VPS اغلب مسدود است. این کار یک خروجی اختصاصی ReaLink به VPS شما می‌دهد. اتصال SSH شما مستقیم می‌ماند؛ فقط ابزارهایی که به آن اشاره می‌کنید از ReaLink استفاده می‌کنند.',
  'vps.status':         'وضعیت',
  'vps.state.none':     'راه‌اندازی نشده',
  'vps.state.pending':  'در حال آماده‌سازی…',
  'vps.state.active':   'فعال',
  'vps.state.revoking': 'در حال غیرفعال‌سازی…',
  'vps.state.revoked':  'غیرفعال',
  'vps.state.error':    'خطا',
  'vps.provisioning':   'در حال آماده‌سازی دستیار VPS شما…',
  'vps.request':        'درخواست دستیار VPS',
  'vps.cmdLabel':       'این یک دستور را روی VPS خود اجرا کنید (در جلسه SSH بچسبانید):',
  'vps.copy':           'کپی دستور نصب',
  'vps.after':          'پس از پایان، اجرا کنید:  source /usr/local/realink-vps-helper/proxy.env  سپس ابزار خود (مثل claude) را اجرا کنید.',
  'vps.revoke':         'غیرفعال کردن / لغو',
  'vps.revokeTitle':    'دستیار VPS غیرفعال شود؟',
  'vps.revokeConfirm':  'این کار هویت ReaLink مربوط به VPS را لغو می‌کند. برای حذف کامل، دستور uninstall را هم روی VPS اجرا کنید.',
  'vps.copiedTitle':    'کپی شد',
  'vps.copiedBody':     'دستور نصب کپی شد. آن را در جلسه SSH ی VPS خود بچسبانید.',
  'vps.errorTitle':     'دستیار VPS',
  'vps.disclaimer':     'این کار VPN یا حالت هوشمند شما را تغییر نمی‌دهد و Termius را مسیریابی نمی‌کند. فقط یک پروکسی محلی روی VPS شما برای ابزارهایی که صریحاً از آن استفاده می‌کنند تنظیم می‌کند.',
