...
| `heartbeat.ps1` | Posts the same JSON schema the Linux gateway posts to `public/starlink-heartbeat.php` — no backend change needed. Every ~33s via Scheduled Task. Backs off (skips, doesn't crash-loop) when there's no internet at all. |
