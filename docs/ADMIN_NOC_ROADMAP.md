...
| DNS + HTTPS for `api.realgram.no`/`admin.realgram.no` (fortsatt kun placeholder-innhold bak) | **Live** (DNS+cert — ikke kode) | — | — | — | 2026-07-18 | SAN-cert dekker begge navn, bekreftet `openssl x509 -ext subjectAltName`; `https://api.realgram.no/` → 503 (tiltenkt stub), `https://admin.realgram.no/` → 200 "coming soon" | certbot gjorde samme `listen 443 ssl`-feil som første runde — fikset til `127.0.0.1:8462`/`8463` samme mønster, se `DECISIONS.md` |
