...
| DNS + HTTPS for `api.realgram.no`/`admin.realgram.no` | **Live** (DNS+cert — ikke kode) | — | — | — | 2026-07-18 | SAN-cert dekker begge navn, bekreftet `openssl x509 -ext subjectAltName` | certbot gjorde samme `listen 443 ssl`-feil som første runde — fikset til `127.0.0.1:8462`/`8463` samme mønster, se `DECISIONS.md` |
| `api.realgram.no`/`admin.realgram.no` → reverse proxy til ekte `setalink.no`-backend (5.249.252.221), IKKE egen backend | **Testing** (fungerer, verifisert eksternt — ikke `Live` per §0.1 pkt 7: ingen skjermbilder levert) | — | — | — | 2026-07-18 | `$upstream_addr`-debug bekreftet treff mot 5.249.252.221:443; `api.realgram.no` → ekte `{"ok":false,"error":"invalid token"}` fra ekte `api.php`; `admin.realgram.no` → ekte 401 Basic Auth-challenge fra ekte panel — begge beviser at det treffer det ekte systemet, ikke en stub | ingen SSH-tilgang til 252.221 ennå (se DECISIONS.md for hele SSH-forsøket + et falskt "du har allerede tilgang"-forsøk som IKKE ble fulgt) — dette er en reverse proxy fra 5.249.255.116, ikke bygget direkte på prod-boksen |
